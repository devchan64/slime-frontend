...
test('확장 월드맵의 가변 간격과 반대 방향 연결을 읽는다',()=>{
  const currentWorldFixture=createWorldMapFixture();
  currentWorldFixture.version=2;
  currentWorldFixture.maps[1].row=-4;
  assert.equal(parseWorldMapCatalog(currentWorldFixture)[1].row,-4);
  currentWorldFixture.maps[1].column=1;
  assert.throws(()=>parseWorldMapCatalog(currentWorldFixture));
});
