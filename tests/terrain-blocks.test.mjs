...
const createTestBlock=(currentBlockOverrides={})=>({id:'a',column:0,row:0,layer:0,offsetHeight:0,height:48,shape:'full',material:'wall',walkable:false,...currentBlockOverrides});
...
test('적층은 중간 윗면을 제거하고 전체 높이를 유지한다',()=>{
 ...
 assert.equal(Math.max(...currentSurfaceFaces.flatMap(currentSurfaceFace=>currentSurfaceFace.vertices.map(currentVertexPoint=>currentVertexPoint.height))),96);
});
...
test('네 방향 경사는 낮은 경계에서 높은 경계까지 이어진다',()=>{
 for(const [currentHighSide,currentLowPoint,currentHighPoint] of [['east',[0,.5],[1,.5]],['west',[1,.5],[0,.5]],['north',[.5,1],[.5,0]],['south',[.5,0],[.5,1]]]) {
  ...
  assert.equal(resolveBlockSurfaceHeight(currentRampBlock,...currentHighPoint),48);
 }
});
