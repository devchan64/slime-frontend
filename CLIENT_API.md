...
텍스트 클라이언트의 `bag`는 최신 상태를 조회해 품목 ID·수량을 표시하고 `useAction`이 있는 회복 소모품에만 `use-item 품목ID`를 안내한다. `first-aid`는 응급처치 API에 ID·버전만 보내고, `use-item 품목ID`는 소모품 사용 API에 `itemId`를 추가한다. 성공 시 최신 HP·상태를 표시한다. 사용 조건과 비용은 서버가 검증하며 클라이언트는 회복량·소비 수량을 지정하지 않는다. 붕대는 직접 사용 안내 대상이 아니며 `first-aid`로 사용한다.
