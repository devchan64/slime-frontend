...
`scout 몬스터ID`는 `/v1/game/skills/scout`에 정찰 명령을 보낸다. 결과의 성공 여부·소비 FP·서버 관측 인원 구간·위험도·관측/만료 시각을 표시한 뒤 갱신된 상태를 출력한다. 정찰 실패도 FP를 소비한 정상 응답이므로 자동 반복하지 않는다. 다시 시도하려면 명령을 직접 입력한다. 관측값은 응답 시점의 기록이며 이후 상태 조회나 heartbeat에서 재사용하지 않는다. 거리·성공률·위험도는 클라이언트에서 추정하지 않는다.

텍스트 클라이언트의 `bag`는 최신 상태를 조회해 품목 ID·수량을 표시하고 `useAction`이 있는 회복·개인 표식 소모품에 `use-item 품목ID`를 안내한다. `first-aid`는 응급처치 API에 ID·버전만 보내고, `use-item 품목ID`는 소모품 사용 API에 `itemId`를 추가한다. 성공 시 최신 HP·상태를 표시한다. 사용 조건과 비용은 서버가 검증하며 클라이언트는 회복량·소비 수량을 지정하지 않는다. 붕대는 직접 사용 안내 대상이 아니며 `first-aid`로 사용한다.

`PLACE_MARKER` 가방 행동은 경로/광원 종류·유지 시간·현재 타일 설치를 안내한다. 같은 소모품 API를 사용하고 좌표를 보내지 않는다. 필드 상태 출력에는 본인의 현재 맵 표식 중 응답의 `serverTime`에 유효한 위치와 만료 시각만 표시한다. 터미널 출력은 해당 서버 시각의 기록이며 자동 갱신 화면이 아니다. 최신 유효 목록은 `state`로 다시 조회한다.

`skills`는 최신 보유 스킬의 ID·레벨과 전투 슬롯을 조회합니다. `loadout physical_activity`처럼 지정하면 슬롯 전체를 해당 목록으로 교체하고, `loadout clear`는 모두 해제합니다. 전투·조우 중에는 변경할 수 없습니다. 보유 여부와 슬롯 한도는 서버에서 최종 검증하며, 이 명령은 스킬 성장이나 즉시 실행 명령이 아닙니다.

`skills`에는 서버가 제공한 액션의 필요 레벨·AP·위력 배율·필요 장비도 표시됩니다. ‘레벨 조건 충족’은 현재 전투에서 즉시 사용할 수 있다는 뜻이 아닙니다. 실제 사용 여부는 전투 응답의 AP·장비·턴·대상 조건을 따릅니다. 0레벨의 효과 미활성 상태와 기존 사용 잠금 기록도 함께 표시합니다.

`hunts`는 본인 사냥 원장의 누적 수량과 기록 50개를 조회합니다. 다음 페이지가 있으면 표시된 `hunts 커서`를 입력합니다. 조회는 게임 상태를 교체하지 않습니다. 원장 도입 이전의 기록은 포함되지 않을 수 있으며, 처치량을 클라이언트에서 등록하거나 수정하지 않습니다.


`rewards [다음커서]`는 계정 보관함을 조회합니다. `rewards claim-all`은 아직 펼치지 않은 페이지를 포함한 유효 보상을 재료별 합산하여 수령하고 게임 상태를 갱신합니다. 만료 보상은 서버에서 제외합니다. 이미 수령한 보상은 재요청해도 중복 지급되지 않으며, 그사이 새로 입고된 보상은 다음 요청에 포함됩니다. 네트워크 오류 시 자동 재시도하지 않으므로 보관함을 다시 확인한 뒤 명령을 재실행하세요.
