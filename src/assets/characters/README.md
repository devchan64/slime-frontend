...
기본 외형 단일 이미지 v1은 폐기했다. 캐릭터 설정 초상화는 standing-v2의 `idle.down_right` 4프레임을 프레임당 400ms로 반복 재생한다. 발 anchor를 정렬하며 다른 초상화는 `down_right.0`을 표시한다. 별도 초상화 이미지를 복제하지 않는다.
