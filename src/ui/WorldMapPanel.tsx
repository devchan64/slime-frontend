...
export function WorldMapPanel({gameSessionClient,currentMapIdentifier}:{gameSessionClient:Client;currentMapIdentifier:string}) {
  ...
  return <section class="world-map-panel">
    ...
    {worldRequestNotice ? <div role="alert"><p>{noticeText(worldRequestNotice,currentWorldLocale,translateWorldText)}</p><button onClick={()=>setWorldRequestAttempt(worldRequestAttempt+1)}>{translateWorldText('app.worldMapRetry')}</button></div> : !worldMapNodes.length ? <p role="status">{translateWorldText('app.worldMapLoading')}</p> : <>
      <button class="secondary" onClick={()=>setSelectedMapIdentifier(currentMapIdentifier)}>{translateWorldText('app.worldMapCurrent')}</button>
      <div class="world-map-scroll" tabIndex={0} aria-label={translateWorldText('app.worldMap')}>
        <div class="world-map-canvas" style={{width:worldCanvasWidth,height:worldCanvasHeight}}>
          ...
          {worldMapNodes.map(currentWorldNode=>{const currentNodeCenter=worldNodeCenter(currentWorldNode);return <button key={currentWorldNode.id} class={`world-map-node ${currentWorldNode.id===currentMapIdentifier?'is-current':''}`} aria-pressed={currentWorldNode.id===selectedMapIdentifier} onClick={()=>setSelectedMapIdentifier(currentWorldNode.id)} style={{left:currentNodeCenter.x-WORLD_MAP_NODE_WIDTH/2,top:currentNodeCenter.y-WORLD_MAP_NODE_HEIGHT/2,width:WORLD_MAP_NODE_WIDTH,height:WORLD_MAP_NODE_HEIGHT}}>
