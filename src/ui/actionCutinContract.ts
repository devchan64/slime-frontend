...
export function parseActionCutinEvent(currentUnknownEvent: unknown): ActionCutinEvent {
  const currentEventRecord = requireExactCutinFields(currentUnknownEvent,ACTION_CUTIN_EVENT_FIELDS);
  ...
  const currentAppearanceRecord = currentEventRecord.appearance as Record<string,unknown> | null;
  if (currentAppearanceRecord?.kind === 'character') {
    requireExactCutinFields(currentAppearanceRecord,CHARACTER_APPEARANCE_FIELDS);
    const currentAppearanceGroups = requireExactCutinFields(currentAppearanceRecord.groups,CHARACTER_GROUP_FIELDS);
    for (const currentFieldName of CHARACTER_GROUP_FIELDS) requireCutinTextValue(currentAppearanceGroups[currentFieldName]);
  } else if (currentAppearanceRecord?.kind === 'monster') {
    requireExactCutinFields(currentAppearanceRecord,MONSTER_APPEARANCE_FIELDS);
    requireCutinTextValue(currentAppearanceRecord.group);
